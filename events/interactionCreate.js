...
module.exports = {
  ...
  async execute(client, interaction) {
    if (interaction.isCommand()) {
      ...
      if (cmd.types.includes("SLASH_GLOBAL")) {
        return cmd.execute(client, interaction)
      }

      interaction.guild.channels.cache.get("997194713927000174").send(`**${interaction.member.user.tag}** использовал команду **/${interaction.commandName}**`)
